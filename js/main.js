mapboxgl.accessToken =
  "pk.eyJ1IjoiZmpvaG5zODg4IiwiYSI6ImNsaGh6eXo1dDAzMDMzbW1td3BqOXFoaDMifQ.RBC_0mpQ25-GRAZDA0E0oA";
...
function elongatePaths(feature, segmentSize = 0.5) {
  ...
  return {
    type: "Feature",
    geometry: { type: "LineString", coordinates: elongated_coords },
  };
}
...
map.on("load", () => {
  ...
  const mbglcc = document.getElementsByClassName(
  ...
  <button id="terrain-btn" class="light-v11" title="Change Terrain"><i class="fa-solid fa-layer-group"></i></button>
...
map.on("load", () => {
  ...
  function featureDebug(e) {
    ...
    const features = map.queryRenderedFeatures(e.point, {
      layers: ["estimation", "real", "glpath_outline"],
    });
  ...
  debugButton.addEventListener("click", async () => {
    ...
    if (debugData === null) {
      const response = await fetch("data/debug.json");
    ...
    if (!debugButton.classList.contains("on")) {
      ...
      debugButton.insertAdjacentHTML(
        "beforeend",
...
class PuppetMaster {
  ...
  constructor(manifest) {
    ...
    this.#puppetPositions = {
      slow: turf.multiPoint(),
      medium: turf.multiPoint(),
      fast: turf.multiPoint(),
    };
    this.#timerSlow = null;
    this.#timerMed = null;
    this.#timerFast = null;
  ...
  findPuppet(mouseCoord, listName) {
    ...
    this.#puppets[listName].forEach((puppet) => {
      ...
      const pPoint =
        puppet.curPosition !== null ? puppet.curPosition : puppet.nextPosition;
  ...
  setupPopups(listName) {
    ...
    map.on("mouseenter", listName, (e) => {
      ...
      const description = `<p>Nationality: ${vi.Nationality}</p>
  ...
  addVesselsToLists(delay) {
    const innerFunc = function () {
      ...
      while (
        this.#curIndex < this.#manifest.length &&
        this.#manifest[this.#curIndex].Date.isSame(this.#date)
      ) {
        const v = this.#manifest[this.#curIndex];
        const from = v["Where From"];
        !collisionMap.has(from)
      ...
      for (const collisions of [...collisionMap.values()].sort(compareFn)) {
        ...
        collisions.forEach((v) => {
          if (v["Vessel Type"] === "Schooner") {
            ...
            v["Vessel Type"] === "Barkentine" ||
            v["Vessel Type"] === "Brigantine"
          ) {
  ...
  reinitLayers() {
    ...
    */
    if (map.getSource("slow") !== undefined) return;
